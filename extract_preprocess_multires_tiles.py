...
def get_args():
    ...
    argparser.add_argument(
        ...
        help="Output directory to save graph files for each slide.",
        required=False,
    )
    ...
    argparser.add_argument(
        ...
        type=float,
        default=None,
        help="Ratio of the slide to be used for tiling.",
        required=False,
    )
...
def process_slide(
    ...
    nuclei_detector = None
    feature_extractor = None
    knn_graph_builder = None
    if options["compute_graph"]:
        ...
        d_graph = dict(
            zip(options["spacing"].keys(), [[] for _ in range(len(options["spacing"]))])
        )
    ...
    for batch in loader:
        ...
        for key in batch.keys():
            ...
            if key == "target":
                ...
                current_d_len = d_coords["x"].shape[0]
                if idx_coords >= current_d_len - batch_size:
            ...
            if options["compute_graph"]:
                for img in x_batch:
                    img = img.numpy()
                    try:
                        nuclei_map, _ = nuclei_detector.process(img)
                        features = feature_extractor.process(img, nuclei_map)
                        cell_graph = knn_graph_builder.process(nuclei_map, features)
                    except ValueError:
                        cell_graph = None
                    d_graph[key].append(cell_graph)

        pbar.update(options["batch_size"])
    pbar.close()

    if options["compute_graph"]:
        for key, graphs in d_graph.items():
            odir = os.path.join(options["graph_path"], image_name)
            if not os.path.exists(os.path.join(odir)):
                os.makedirs(odir)
            graph_idxs = np.where(np.array(graphs) != None)[0]
            _graphs = [graphs[i] for i in graph_idxs]
            if _graphs == []:
                continue
            dgl.save_graphs(
                os.path.join(odir, image_name + "_" + key + ".bin"),
                _graphs,
            )
            # Save graph indices
            np.save(
                os.path.join(odir, image_name + "_" + key + "_idxs.npy"),
                graph_idxs,
            )
...
def process_slides(
    ...
    for image_idx, image_file in enumerate(image_files):
        ...
        try:
            ...
            telegramBot.send(
                f"Image {image_name} not found in labels csv, skipping",
            )
            continue
...
def main():
    ...
    files = get_files(
        ...
        ratio=config.preprocess.ratio if args.ratio is None else args.ratio,
    ...
    options = {
        ...
        "compute_graph": config.preprocess.compute_graph,
        "graph_path": args.output_graph_dir,
    }

    if options["compute_graph"]:
        assert (
            options["graph_path"] is not None
        ), "Graph path must be provided if compute_graph is True"
