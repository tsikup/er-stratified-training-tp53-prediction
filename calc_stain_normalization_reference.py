...
def sample_slide(
    ...
    for x_batch in loader:
        ...
        for x_target in x_batch["target"]['img_array']:
...
def calculate_dataset_level_stain_norm(
    ...
    if telegram_key is not None and telegram_id is not None:
        ...
        print("Dataset-level stain normalization reference calculation started.")
        results = list(
            tqdm(
                pool.imap_unordered(call_process_images_dataset_level, tasks),
                desc="dataset-level",
...
def get_tiled_slide_multiprocess(
    ...
    pool.close()
    pool.join()
    print('Done getting tiled slides')
...
def main():
    ...
    slide_extension = config.preprocess.slide_extension
    if not slide_extension.startswith("."):
        slide_extension = "." + slide_extension

    ann_extension = config.preprocess.ann_extension
    ...
    image_files = get_tiled_slide_multiprocess(
        ...
        config.preprocess.ratio if args.ratio is None else args.ratio,
        "wsi",
        slide_extension,
        ann_extension,
    )
    ...
    if args.dataset_image_num is not None and args.dataset_image_num > 0:
        ...
        if dataset_reference_df is not None:
            ...
            print("No dataset reference dataframe to save.")
