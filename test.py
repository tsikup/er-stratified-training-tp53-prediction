...
def main(args, config, model_folder, mode, verbose=True):
    ...
    for version in range(versions):
        ...
        for idx, fold in enumerate(folds):
            ...
            if config.model.classifier == "clam":
                ...
                model = DSMIL_PL.load_from_checkpoint(model_ckpt, strict=False)
            elif config.model.classifier == "admil":
                model = AttentionDeepMIL_PL.load_from_checkpoint(
                    model_ckpt, strict=False
                )
            elif config.model.classifier == "mamil":
                model = MAMIL_PL.load_from_checkpoint(model_ckpt, strict=False)
            elif "minet" in config.model.classifier:
                model = MINet_PL.load_from_checkpoint(model_ckpt, strict=False)
            elif "csmil" in config.model.classifier:
                model = CSMIL_PL.load_from_checkpoint(model_ckpt, strict=False)
            elif "dtfd" in config.model.classifier:
                model = DTFD_PL.load_from_checkpoint(model_ckpt, strict=False)
            elif "mmil" in config.model.classifier:
                model = MMIL_PL.load_from_checkpoint(model_ckpt, strict=False)
            else:
            ...
            tmp_predictions = [
                [
                    item[0].detach().cpu().numpy().squeeze(),
                    item[1].detach().cpu().numpy().squeeze().astype(np.uint8),
                    item[2][0],
                ]
                for item in tmp_predictions
            ]
            tmp = pd.DataFrame(
                tmp_predictions, columns=["preds", "label", "slide_name"]
            )
...
if __name__ == "__main__":
    ...
    main(args, config, model_folder, mode, verbose=True)
